...
function generateGatewaySignature() {
  ...
  var _cvp=parseFloat((document.getElementById('curVacancy')||{}).value)||0,_pvp=parseFloat((document.getElementById('pfVacancy')||{}).value)||0;
  var curVacLoss=Math.round(curIncome*_cvp/100),pfVacLoss=Math.round(pfIncome*_pvp/100);
  var curOtherTot=(curOtherIncome||[]).reduce(function(s,o){return s+(+o.amount||0);},0);
  var pfOtherTot=(curOtherIncome||[]).reduce(function(s,o){return s+(+(o.pfAmount||o.amount)||0);},0);
  var curEGI=curIncome-curVacLoss+curOtherTot,pfEGI=pfIncome-pfVacLoss+pfOtherTot;
  ...
  var finPage=page('05',
    ...
    +'<table style="flex:1;">'
      ...
      +'<tbody>'
        ...
        +(curVacLoss>0?'<tr><td style="color:#c0392b">Less: Vacancy/Credit Loss</td><td class="tr" style="color:#c0392b">('+fmt(curVacLoss)+')</td><td class="tr" style="color:#c0392b">('+fmt(pfVacLoss)+')</td></tr>':'')
        +(curOtherRows||'')
        +'<tr class="tfoot-row"><td>Effective Gross Income</td><td class="tr">'+fmt(curEGI)+'</td><td class="tr">'+fmt(pfEGI)+'</td></tr>'
        ...
        +'<tr class="tfoot-row" style="background:#E0ECE0!important;"><td style="color:#1E6040;">Net Operating Income</td><td class="tr" style="color:#1E6040;">'+fmt(noi)+'</td><td class="tr" style="color:#1E6040;">'+fmt(pfEGI-pfExpTotal)+'</td></tr>'
      +'</tbody>'
    +'</table>'
    +'<div class="pg-foot"><span class="pg-foot-l">Gateway Real Estate Advisors · Confidential</span><span class="pg-foot-r">'+prepared+'</span></div>'
  );
...
function generateGatewayInstitutional() {
  try {
    ...
    txt(s1, propName, RX, 1.22, 5.1, 1.4, { fontSize:52, color:W, fontFace:'Cambria', bold:true, lineSpacingMultiple:1.1 });
    txt(s1, 'APARTMENTS', RX, 2.72, 5.1, 0.56, { fontSize:36, color:G, fontFace:'Cambria' });
    ...
    [0,1].forEach(function(col) {
      [0,1].forEach(function(row) {
        ...
        txt(s1, kpis1[idx].v||'—', kx, ky+0.28, kpiW1, 0.46, { fontSize:34, color:W, fontFace:'Cambria', align:'center', valign:'middle' });
      });
    });
    footer(s1, '1');
    ...
    txt(s3, propName, lx3, cY3, 8.0, 0.66, { fontSize:38, color:N, fontFace:'Cambria', bold:true });
    ...
    var kpiCards3 = [
      ...
      { l:'OCCUPANCY RATE',          v:occ,               d:(units&&units!=='0' ? units+' of '+units+' Units Occupied' : 'Current Occupancy') },
    ];
    kpiCards3.forEach(function(kpi, i) {
      ...
      txt(s3, kpi.v, kRX, ky+0.52, kRW, cardH3-0.9, { fontSize:52, color:W, fontFace:'Cambria', align:'center', valign:'middle' });
    ...
    txt(s8, 'Meet the Team', 0.38, 1.04, 12.54, 0.6, { fontSize:38, color:N, fontFace:'Cambria', bold:true });
    ...
    agts.slice(0,2).forEach(function(ag, i) {
      ...
      // Card: cream base + gold top accent + navy band (1.78" per spec)
      box(s8, cx, cY8, cW8, cH8, W);
      box(s8, cx, cY8, cW8, 0.06, G);
      box(s8, cx, cY8+0.06, cW8, 1.78, N);
      // Initials avatar — centered in navy band
      var iX = cx+cW8/2-0.52, iY = cY8+0.06+(1.78-1.04)/2;
      box(s8, iX, iY, 1.04, 1.04, N2);
      box(s8, iX, iY, 1.04, 0.042, G);
      var inits = (ag.name||'').split(' ').map(function(w){ return w[0]||''; }).join('').slice(0,2).toUpperCase();
      txt(s8, inits, iX, iY, 1.04, 1.04, { fontSize:30, color:W, fontFace:'Cambria', align:'center', valign:'middle' });
      // Name and title below navy band (spec: 21pt Navy, 8pt charSpacing:1.5)
      txt(s8, ag.name||'', cx, cY8+2.0, cW8, 0.42, { fontSize:21, color:N, fontFace:'Cambria', align:'center', valign:'middle' });
      txt(s8, (ag.title||'').toUpperCase(), cx, cY8+2.48, cW8, 0.24, { fontSize:8, color:G, fontFace:'Calibri', charSpacing:1.5, align:'center' });
      box(s8, cx+0.38, cY8+2.82, cW8-0.76, 0.022, G);
      txt(s8, ag.phone||'', cx, cY8+2.96, cW8, 0.36, { fontSize:17, color:N, fontFace:'Calibri', bold:true, align:'center' });
      txt(s8, ag.email||'', cx, cY8+3.38, cW8, 0.28, { fontSize:10.5, color:GR, fontFace:'Calibri', align:'center' });
      txt(s8, ag.licenses||ag.lic||'', cx, cY8+3.72, cW8, 0.26, { fontSize:8.5, color:ST, fontFace:'Calibri', italic:true, align:'center' });
      box(s8, cx+0.38, cY8+4.08, cW8-0.76, 0.022, C2);
      txt(s8, ag.bio||ag.description||'', cx+0.22, cY8+4.22, cW8-0.44, cH8-4.34, { fontSize:9.5, color:CH, fontFace:'Calibri', lineSpacingMultiple:1.35, valign:'top' });
    });
    footer(s8, '8');
    ...
    txt(s9, 'Gateway Real Estate Advisors', 0.38, 1.04, 12.54, 0.56, { fontSize:34, color:N, fontFace:'Cambria', bold:true });
    ...
    sts9.forEach(function(st, i) {
      ...
      txt(s9, st.v, sx, 3.22, stW9, 0.58, { fontSize:50, color:W, fontFace:'Cambria', align:'center', valign:'middle' });
...
function generateOM() {
  ...
  var n = function(id) { var el = document.getElementById(id); return el ? (+el.value || 0) : 0; };
  try {
    if (typeof generateOfferingMemorandum !== 'function') {
      alert('OM generator not loaded — please refresh the page.');
      return;
    }

    // ── Financial roll-up ──────────────────────────────────────────────────
    var curVacPct   = parseFloat((document.getElementById('curVacancy') || {}).value) || 0;
    var pfVacPct    = parseFloat((document.getElementById('pfVacancy')  || {}).value) || 0;
    var curGRI      = n('curIncome');
    var pfGRI       = n('pfIncome');
    var curOtherTot = (curOtherIncome || []).reduce(function(s,e){return s+(+e.amount||0);},0);
    var pfOtherTot  = (pfOtherIncome  || []).reduce(function(s,e){return s+(+e.amount||0);},0);
    var curVacLoss  = Math.round(curGRI * curVacPct / 100);
    var pfVacLoss   = Math.round(pfGRI  * pfVacPct  / 100);
    var curEGI      = curGRI + curOtherTot - curVacLoss;
    var pfEGI       = pfGRI  + pfOtherTot  - pfVacLoss;
    var curExpTot   = (curExpenses || []).reduce(function(s,e){return s+(+e.amount||0);},0);
    var pfExpTot    = (pfExpenses  || []).reduce(function(s,e){return s+(+e.amount||0);},0);
    var curNOI      = curEGI - curExpTot;
    var pfNOI       = pfEGI  - pfExpTot;
    var askingPrice = n('askingPrice');
    var totalUnits  = n('totalUnits');
    var totalSqFt   = (unitData || []).reduce(function(s,u){return s+(u.units||0)*(u.sqft||0);},0);

    // ── Income schedule ────────────────────────────────────────────────────
    var incomeItems = [];
    incomeItems.push({label:'Gross Rental Income', current:curGRI, proforma:pfGRI});
    (curOtherIncome || []).forEach(function(o, i) {
      var pf = (pfOtherIncome || [])[i] || {};
      incomeItems.push({label:o.name||'Other Income', current:+o.amount||0, proforma:+pf.amount||+o.amount||0});
    });
    if (curVacLoss > 0 || pfVacLoss > 0) {
      incomeItems.push({label:'Vacancy Loss (' + curVacPct.toFixed(1) + '%)', current:-curVacLoss, proforma:-pfVacLoss});
    }
    incomeItems.push({label:'Effective Gross Income', current:curEGI, proforma:pfEGI});

    // ── Expense schedule ───────────────────────────────────────────────────
    var expenseItems = (curExpenses || []).filter(function(e){return e.name;}).map(function(e, i) {
      return {
        label:    e.name,
        current:  +e.amount || 0,
        proforma: +((pfExpenses||[])[i] && (pfExpenses||[])[i].amount) || +e.amount || 0,
      };
    });
    expenseItems.push({label:'Total Expenses', current:curExpTot, proforma:pfExpTot});

    // ── Investment highlights ───────────────────────────────────────────────
    var highlights = [v('hl1'),v('hl2'),v('hl3'),v('hl4')].filter(Boolean).map(function(h) {
      var sep = h.indexOf(' - ');
      return sep !== -1
        ? {title:h.substring(0,sep), description:h.substring(sep+3)}
        : {title:h, description:''};
    });
    ...
    var data = {
      property: {
        name:         (v('propName1') + ' ' + v('propName2')).trim() || 'Property Name',
        address:      v('address'),
        type:         v('propType') || 'Multifamily',
        units:        totalUnits,
        yearBuilt:    v('yearBuilt'),
        stories:      v('buildings') || '',
        lotSize:      v('lotSize'),
        parking:      v('parking'),
        buildingSize: totalSqFt,
        images: {
          hero:   (photos || [])[0] || null,
          photos: (photos || []).filter(Boolean),
        },
        amenities:   v('features')
          ? v('features').split(',').map(function(f){return f.trim();}).filter(Boolean)
          : [],
        description: v('propDesc'),
        highlights:  highlights,
        execSummary: v('execDesc'),
        callout:     v('callout'),
        occupancy:   parseFloat(v('occupancy')) || 100,
      },
      financials: {
        askingPrice:   askingPrice,
        pricePerUnit:  totalUnits > 0 ? Math.round(askingPrice / totalUnits) : n('pricePerUnit'),
        pricePerSF:    totalSqFt  > 0 ? Math.round(askingPrice / totalSqFt * 100) / 100 : 0,
        capRate:       parseFloat(v('capRate'))  || 0,
        grm:           parseFloat(v('grm'))      || 0,
        noi:           curNOI > 0 ? curNOI : n('noi'),
        grossRevenue:  curGRI,
        totalExpenses: curExpTot,
        occupancy:     parseFloat(v('occupancy')) || 100,
        incomeItems:   incomeItems,
        expenseItems:  expenseItems,
        proforma: {
          grossRevenue:  pfGRI,
          totalExpenses: pfExpTot,
          noi:           pfNOI,
          capRate:       askingPrice > 0 ? Math.round(pfNOI / askingPrice * 10000) / 100 : 0,
        },
      },
      unitMix: (unitData || [])
      ...
      market: {
        ...
        medianIncome: v('medianIncome'),
        unemployment: v('unemployment'),
        medianAge:    v('medianAge'),
        ...
        drivers: [
          ...
          {title:v('drv2Title'), description:v('drv2Desc')},
          {title:v('drv3Title'), description:v('drv3Desc')},
      ...
      brokerage: {
        ...
        disclaimer:   v('disclaimer') ||
          'This Offering Memorandum is intended solely for the use of the party to whom it is delivered. ' +
          'The information herein has been obtained from sources believed reliable but has not been independently ' +
          'verified. Prospective purchasers should make their own investigations and inquiries. All projections ' +
          'are forward-looking estimates; actual results may vary materially.',
